...
(async () => {
  ...
  const kpiVal = await page.$eval("text=إجمالي التحاليل", (el) =>
    el.closest(".glass")?.querySelector(".text-3xl")?.textContent?.trim(),
  );
  ...
  for (const demoMod of ["image", "stego"]) {
    ...
    await test(`${demoMod}: demo analysis runs and shows results`, async () => {
      ...
      await page.waitForSelector("text=VERDICT", { timeout: 8000 });
      const body = await page.textContent("main");
      if (!body.includes("VERDICT") && !body.includes("النتائج"))
        throw new Error("No results shown");
    });
  }

...
(async () => {
  ...
  const desktopErrors = [];
  dp.on('console', m => { if (m.type()==='error') desktopErrors.push(m.text()); });
  ...
  await test('MOD-01 Phishing: يرصد رسالة تصيد عربية', async () => {
    ...
    await ep.fill('textarea', 'عزيزي العميل! حسابك البنكي معلق. يجب تحديث بياناتك فوراً وإلا سيتم إغلاق الحساب. اضغط هنا: http://fake-bank.com/login لتجنب الحجب النهائي خلال 24 ساعة.');
  ...
  await test('MOD-02 URL: يرصد رابط مشبوه', async () => {
    ...
    await ep.fill('textarea', 'http://paypa1-secure-account-verify.tk/login?redirect=original-site.com');
  ...
  await test('MOD-02 URL: يقبل رابط آمن', async () => {
    ...
    await ep.waitForSelector('text=VERDICT', { timeout: 8000 });
    const body = await ep.textContent('main');
    if (!body.includes('نظيف') && !body.includes('تهديد')) throw new Error('No verdict for google.com');
  });
  ...
  await test('MOD-06 IOC: يستخرج IPv4 وهاشات', async () => {
    ...
    await ep.waitForLoadState('networkidle');
    await ep.waitForSelector('textarea');
    await ep.fill('textarea', 'Suspicious connection to 185.220.101.45 from 10.0.0.1\nMD5: d41d8cd98f00b204e9800998ecf8427e\nSHA256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855\nDomain: evil-c2.xyz contacted');
    await ep.waitForTimeout(300);
    await ep.click('button:has-text("تشغيل التحليل"):not([disabled])');
    await ep.waitForSelector('text=VERDICT', { timeout: 8000 });
    const body = await ep.textContent('main');
  ...
  await test('MOD-10 MITRE: يعيّن تقنيات ATT&CK', async () => {
    ...
    await ep.waitForLoadState('networkidle');
    await ep.waitForSelector('textarea');
    await ep.fill('textarea', 'PowerShell encoded command executing registry persistence. Base64 obfuscation with C2 beacon to bitcoin wallet.');
    await ep.waitForTimeout(300);
    await ep.click('button:has-text("تشغيل التحليل"):not([disabled])');
    await ep.waitForSelector('text=VERDICT', { timeout: 8000 });
    const body = await ep.textContent('main');
    if (!body.includes('T1') && !body.includes('تقنية')) throw new Error('No MITRE techniques found');
  });
  ...
  await test('MOD-03 Image: تحليل نموذجي يعمل', async () => {
    ...
    await ep.waitForSelector('text=VERDICT', { timeout: 8000 });
    const body = await ep.textContent('main');
    if (!body.includes('EXIF') && !body.includes('صورة') && !body.includes('بصمة')) throw new Error('No image analysis results');
  });
  ...
  await test('MOD-07 Stego: تحليل نموذجي يعمل', async () => {
    ...
    await ep.waitForSelector('text=VERDICT', { timeout: 8000 });
    const body = await ep.textContent('main');
    if (!body.includes('LSB') && !body.includes('خفاء') && !body.includes('Entropy')) throw new Error('No stego results');
  });
  ...
  await test('FindingCard: يفتح/يغلق الأدلة بالضغط', async () => {
    ...
    await up.fill('textarea', 'اضغط هنا فوراً وأدخل كلمة مرورك لتجنب إغلاق الحساب البنكي خلال 24 ساعة!');
    await up.waitForTimeout(300);
  ...
  await test('أداء التحليل: phishing < 4 ثوان', async () => {
    ...
    await pp.fill('textarea', 'urgent: click here now to verify your account credentials immediately');
  ...
  console.log('══════════════════════════════════════');
  const allErrors = [...desktopErrors, ...mobileErrors, ...engineErrors]
    .filter(e => !e.includes('favicon') && !e.includes('source map') && !e.includes('DevTools'));

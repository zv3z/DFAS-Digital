...
export function TopBar({ onMenu }: { onMenu: () => void }) {
  ...
  const crumb =
    NAV.find((n) => (n.to === "/" ? path === "/" : path.startsWith(n.to)))?.label ?? "صفحة";
...
function StatusPill({
  ...
  return (
    <div
      className={`hidden md:flex items-center gap-2 px-2.5 py-1 rounded-md border text-[10px] font-mono ${map[color]}`}
    >

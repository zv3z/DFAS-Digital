...
const SEV_MAP: Record<Severity, { bg: string; text: string; border: string; label: string }> = {
  CRITICAL: { bg: "bg-critical/15", text: "text-critical", border: "border-critical/40", label: "حرج" },
  HIGH:     { bg: "bg-high/15",     text: "text-high",     border: "border-high/40",     label: "مرتفع" },
  MEDIUM:   { bg: "bg-warning/15",  text: "text-warning",  border: "border-warning/40",  label: "متوسط" },
  LOW:      { bg: "bg-info/15",     text: "text-info",     border: "border-info/40",     label: "منخفض" },
  INFO:     { bg: "bg-muted",       text: "text-muted-foreground", border: "border-border", label: "معلوماتي" },
};
...
export function Counter({ value, suffix }: { value: number; suffix?: string }) {
  ...
  return <span className="font-mono tabular-nums">{v.toLocaleString("en")}{suffix}</span>;
}
    ...
    <div className="relative inline-grid place-items-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-[135deg]">
        <circle cx={size/2} cy={size/2} r={r}
          fill="none" stroke="oklch(0.28 0.025 248)" strokeWidth="10"
          strokeDasharray={`${dash} ${c}`} strokeLinecap="round" />
        <circle cx={size/2} cy={size/2} r={r}
          fill="none" stroke={color} strokeWidth="10"
          strokeDasharray={`${dash} ${c}`} strokeDashoffset={offset}
          strokeLinecap="round"
          style={{ transition: "stroke-dashoffset 1.2s cubic-bezier(.2,.8,.2,1), stroke .3s", filter: `drop-shadow(0 0 8px ${color})` }} />
      </svg>
      <div className="absolute inset-0 grid place-items-center text-center">
        <div>
          <div className="font-mono text-3xl font-bold" style={{ color }}>{Math.round(v)}<span className="text-base">%</span></div>

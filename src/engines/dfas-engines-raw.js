...
const ImageEngine = (() => {
  ...
  function i32(v,o,le){return v.getInt32(o,le);}
  function readStr(v,o,n){let s='';for(let i=0;i<n;i++){const c=v.getUint8(o+i);if(!c)break;s+=String.fromCharCode(c);}return s.trim();}
  function rational(v,o,le){const n=u32(v,o,le),d=u32(v,o+4,le);return d?n/d:0;}
  function sRational(v,o,le){const n=i32(v,o,le),d=i32(v,o+4,le);return d?n/d:0;}
  ...
  function readIFD(v, ifdOff, base, le, tagDict) {
    ...
    for (let i=0;i<n&&p+12<=v.byteLength;i++,p+=12) {
      const tag = u16(v,p,le), type = u16(v,p+2,le), cnt = u32(v,p+4,le);
      const vp = p+8;
      let val = null;
      try {
        if (type===2) {               // ASCII
          ...
          val = cnt===1 ? u16(v,vp,le) :
                cnt===2 ? [u16(v,vp,le),u16(v,vp+2,le)] :
                          (() => { const off=base+u32(v,vp,le),a=[];for(let j=0;j<Math.min(cnt,8);j++)a.push(u16(v,off+j*2,le));return a; })();
        } else if (type===4) {        // LONG
          val = cnt===1 ? u32(v,vp,le) : u32(v,base+u32(v,vp,le),le);
        } else if (type===5) {        // RATIONAL
          const off=base+u32(v,vp,le);
          val = cnt===1 ? rational(v,off,le) : Array.from({length:Math.min(cnt,4)},(_,j)=>rational(v,off+j*8,le));
          ...
          if (cnt<=4) { val=[]; for(let j=0;j<cnt;j++) val.push(v.getUint8(vp+j)); }
          else { const off=base+u32(v,vp,le); val=[]; for(let j=0;j<Math.min(cnt,16);j++) val.push(v.getUint8(off+j)); }
  ...
  function editorSigs(arr) {
    const s = Array.from(arr.slice(0,8192)).map(b=>String.fromCharCode(b)).join('');
    const found = [];
    const checks = [
  ...
  async function readMetadata(file) {
    return new Promise(resolve => {
      ...
      fr.onload = e => {
        ...
        const magic = arr[0]===0xFF&&arr[1]===0xD8?'JPEG':arr[0]===0x89&&arr[1]===0x50?'PNG':arr[0]===0x47&&arr[1]===0x49?'GIF':arr[0]===0x42&&arr[1]===0x4D?'BMP':arr[0]===0x25&&arr[1]===0x50?'PDF':'UNKNOWN';
        const res = { name:file.name, size:file.size, type:file.type||magic, magic, lastModified:file.lastModified, tags:{}, gps:null, editorSigs:[], jpegQuality:null, hasThumbnail:false };

        res.editorSigs  = editorSigs(arr);
        res.jpegQuality = magic==='JPEG' ? jpegQuality(arr) : null;

        if (magic!=='JPEG') { resolve(res); return; }
        ...
        while (pos<arr.length-4) {
          ...
          if (mkr===0xE1 && len>6 && readStr(view,pos+4,4)==='Exif') {
            ...
            if (u16(view,tb+2,le)!==42){pos+=2+len;continue;}
            ...
            if (ifd0['GPSInfoIFDPointer']!=null) {
              ...
              if (lat!==null&&lon!==null) res.gps={lat:lat.toFixed(6),lon:lon.toFixed(6)};
            }
            ...
            try {
              ...
              if (nOff>0&&nOff<tView.byteLength-12) {
                ...
                res.hasThumbnail=ifd1['Compression']!==undefined||ifd1['JPEGInterchangeFormat']!==undefined;
              }
            } catch(_){}
            break;
          }
  ...
  function buildIndicators(meta, file) {
    const t=meta.tags, inds=[];
    let score=0;

    const sw = (t.Software||'').toLowerCase();
    const editors=['photoshop','gimp','lightroom','snapseed','facetune','faceapp','canva','paint','affinity','pixelmator','meitu','capture one'];
    ...
    if (foundEd||meta.editorSigs.length) {
      ...
      inds.push({sev:'CRITICAL',rule:`بصمة تحرير مُكتشفة: ${label}`,det:'EXIF:Software يُثبت استخدام برنامج تحرير — دليل قاطع على التعديل الرقمي',ev:`EXIF:Software = "${t.Software||meta.editorSigs[0].label}"`});
    }
    ...
    if (dOrig&&dMod&&dOrig!==dMod) {
      const parse=s=>{try{return new Date(s.replace(/^(\d{4}):(\d{2}):(\d{2})/,'$1-$2-$3'));}catch{return null;}};
      const d1=parse(dOrig),d2=parse(dMod);
      if (d1&&d2) {
        const diffMs=Math.abs(d2-d1), diffH=(diffMs/3600000).toFixed(1);
        if (diffMs>120000){score+=20;inds.push({sev:'HIGH',rule:`تعارض زمني: DateTimeOriginal ↔ DateTime (Δ ${diffH}h)`,det:`الالتقاط: ${dOrig} | التعديل: ${dMod} — فارق ${diffH} ساعة مؤشر على إعادة الحفظ`,ev:`Original:${dOrig} | Modified:${dMod} | Δ=${diffH}h`});}
      }
    }

    // Thumbnail
    if (!meta.hasThumbnail&&meta.magic==='JPEG') {
      score+=12; inds.push({sev:'HIGH',rule:'Thumbnail EXIF غائب — مؤشر إعادة تصدير',det:'الكاميرات الحديثة تحفظ Thumbnail تلقائياً في IFD1. غيابه يُشير إلى إعادة الحفظ أو التعديل الكامل',ev:'EXIF:IFD1:Thumbnail = NOT FOUND'});
    }

    // GPS
    if (meta.gps) {
      score+=5; inds.push({sev:'MEDIUM',rule:'إحداثيات GPS مُضمَّنة — معلومات الموقع مكشوفة',det:`إحداثيات دقيقة قابلة للتتبع الجغرافي`,ev:`GPS: ${meta.gps.lat}°N, ${meta.gps.lon}°E`});
    }

    // JPEG quality
    if (meta.jpegQuality!==null&&meta.jpegQuality<88) {
      score+=15;inds.push({sev:'MEDIUM',rule:`جودة JPEG منخفضة (~${meta.jpegQuality}%) — مؤشر إعادة ضغط`,det:`الكاميرات الحديثة تنتج JPEG بجودة 92-98%. الجودة ${meta.jpegQuality}% تدل على إعادة الحفظ أو التصدير`,ev:`JPEG Quality Factor ≈ ${meta.jpegQuality}% (Expected: ≥92%)`});
    }

    // Binary editor sigs (separate from EXIF)
    for (const sig of meta.editorSigs) {
      if (!foundEd) {
        score+=25; inds.push({sev:sig.sev,rule:`توقيع ثنائي: ${sig.label}`,det:'رُصد توقيع برنامج التحرير في البايتات الثنائية للملف مباشرةً',ev:`Binary signature found: "${sig.label}"`});
      }
    }
    ...
    if (meta.lastModified&&dOrig) {
      ...
      if (!isNaN(exifD)&&!isNaN(fsD)) {
        ...
        if (diffDays>45) {score+=5;inds.push({sev:'LOW',rule:`فجوة بين تاريخ الملف (FS) وتاريخ EXIF: ${Math.round(diffDays)} يوم`,det:'الفارق الزمني الكبير قد يُشير إلى نسخ ملف قديم أو تلاعب بالتواريخ',ev:`EXIF:${dOrig} | FS:${new Date(meta.lastModified).toISOString().slice(0,10)}`});}
      }
    }
  ...
  function buildTable(meta, hashStr) {
    const t=meta.tags, rows=[];
    const r=(k,v,f='')=>rows.push({k,v:v!=null&&v!==''?String(v):'—',flag:f});
    r('اسم الملف',meta.name);
    r('الحجم',`${(meta.size/1024).toFixed(1)} KB`);
    r('النوع / MIME',meta.type);
    r('التنسيق (Magic Bytes)',meta.magic);
    const w=t.PixelXDimension||t.ImageWidth, h=t.PixelYDimension||t.ImageLength;
    if (w&&h) r('الأبعاد',`${w} × ${h} px`);
    const mk=t.Make,mo=t.Model;
    if (mk||mo) r('الكاميرا / الجهاز',[mk,mo].filter(Boolean).join(' '));
    if (t.Software) {
      const isEd=['photoshop','gimp','lightroom','snapseed','facetune','faceapp','canva'].some(n=>String(t.Software).toLowerCase().includes(n));
      r('برنامج التحرير (EXIF)',t.Software,isEd?'warn':'');
    }
    if (t.DateTimeOriginal) r('تاريخ الالتقاط (DateTimeOriginal)',t.DateTimeOriginal);
    if (t.DateTimeDigitized&&t.DateTimeDigitized!==t.DateTimeOriginal) r('تاريخ الرقمنة',t.DateTimeDigitized);
    if (t.DateTime) {
      const diff=t.DateTime!==t.DateTimeOriginal&&t.DateTimeOriginal;
      r('آخر تعديل (DateTime)',t.DateTime+(diff?' ⚠ يختلف عن الأصل':''),diff?'warn':'');
    }
    if (t.ISOSpeedRatings) r('ISO Speed',`ISO-${Array.isArray(t.ISOSpeedRatings)?t.ISOSpeedRatings[0]:t.ISOSpeedRatings}`);
    if (t.FNumber) r('فتحة العدسة',`f/${typeof t.FNumber==='number'?t.FNumber.toFixed(1):t.FNumber}`);
    ...
    if (t.Flash!==undefined) r('الفلاش',t.Flash===0?'لم يُستخدم':t.Flash===1?'استُخدم':`Code:${t.Flash}`);
    r('إحداثيات GPS',meta.gps?`${meta.gps.lat}°, ${meta.gps.lon}°`:'غير موجودة',meta.gps?'':'');
    r('Thumbnail EXIF',meta.hasThumbnail?'✓ موجود':'⚠ غائب — مؤشر إعادة حفظ',meta.hasThumbnail?'ok':'warn');
    if (meta.jpegQuality!==null) r('جودة JPEG المقدّرة',`~${meta.jpegQuality}% (الافتراضي ≥92%)`,meta.jpegQuality<88?'warn':'ok');
    if (hashStr) r('SHA-256 (مُحسوب آنياً)',hashStr+' [Verified]','ok');
    r('تاريخ تعديل الملف (FS)',new Date(meta.lastModified).toISOString().replace('T',' ').slice(0,19));
    return rows;
  }
  ...
  async function analyze(file) {
    ...
    if (!file) {
      return {
        ...
        indicators:[
          ...
          {sev:'CRITICAL',rule:'ELA — شذوذ مستويات ضغط JPEG',det:'اكتُشفت مناطق في الصورة بمستويات ضغط مختلفة — مؤشر على اللصق والتعديل في الربع السفلي الأيسر',ev:'ELA Anomaly Region: x:1280–1920, y:900–1440 px'},
          {sev:'HIGH',rule:'تعارض زمني: DateTimeOriginal ↔ DateTime (Δ 7.6h)',det:'تاريخ الالتقاط 08:14:22 يختلف عن DateTime 15:52:07 — فارق 7.6 ساعة يدل على إعادة حفظ',ev:'Original:2024:04:10 08:14:22 | Modified:2024:04:10 15:52:07'},
          {sev:'HIGH',rule:'Thumbnail EXIF غائب — مؤشر إعادة تصدير',det:'الكاميرات الحديثة تحفظ Thumbnail تلقائياً. غيابه يُشير إلى تصدير يدوي أو تعديل كامل للملف',ev:'EXIF:IFD1:Thumbnail = NOT FOUND'},
          {sev:'MEDIUM',rule:'جودة JPEG منخفضة (~82%) — مؤشر إعادة ضغط',det:'iPhone ينتج 94-98%. الجودة 82% تدل على إعادة الحفظ بجودة أقل بعد التعديل',ev:'JPEG Quality Factor ≈ 82% (Camera default: 94-98%)'},
          {sev:'MEDIUM',rule:'إحداثيات GPS مُضمَّنة — معلومات الموقع مكشوفة',det:'إحداثيات GPS دقيقة تكشف موقع الالتقاط — قد تستخدم في تتبع الجغرافي',ev:'GPS: 24.713600°N, 46.675300°E (الرياض، SA)'},
        ],
        exifTable:[
        ...
        chips:['Photoshop Detected','ELA Anomaly','Timestamp Mismatch','Thumbnail Missing','GPS Embedded','Re-encoded JPEG'],
        name:'evidence_photo_032.jpg', size:'1,247.3', type:'image/jpeg'
      };
    }
    ...
    try {
      ...
      hashStr = [...new Uint8Array(dig)].map(b=>b.toString(16).padStart(2,'0')).join('').slice(0,32)+'…';
    } catch(_) {}

    const hasRealEXIF = meta.magic==='JPEG' && Object.keys(meta.tags).length > 2;
    const analysis    = hasRealEXIF ? buildIndicators(meta, file) : {score:30, inds:[
      {sev:'MEDIUM',rule:'بيانات EXIF غير موجودة أو محدودة',det:'لا توجد بيانات EXIF قابلة للقراءة — الملف قد يكون مُجرَّداً من البيانات الوصفية أو تالفاً',ev:`Format: ${meta.magic}`}
    ]};
    ...
    if (meta.jpegQuality!==null&&meta.jpegQuality<88) chips.push(`JPEG Quality ${meta.jpegQuality}%`);
    if (meta.tags.DateTimeOriginal&&meta.tags.DateTime&&meta.tags.DateTimeOriginal!==meta.tags.DateTime) chips.push('Timestamp Mismatch');

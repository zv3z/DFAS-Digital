...
const DFAS_DB = (() => {
  ...
  const STORES = {
    ...
    reports: {
      keyPath: "reportId",
      autoIncrement: false,
      indexes: [{ name: "caseId", keyPath: "caseId", opts: { unique: false } }],
    },
  ...
  async function getCases() {
    const a = await getAll("cases");
    return a.filter((c) => !c._deleted).sort((x, y) => y.ts - x.ts);
  }
  ...
  async function getReports() {
    const a = await getAll("reports");
    return a.sort((x, y) => y.ts - x.ts);
  }
  ...
  async function exportAll() {
    ...
    return JSON.stringify(
      { exportedAt: new Date().toISOString(), analyses, cases, reports },
      null,
      2,
    );
  }

...
const IOCEngine = (() => {
  ...
  const SUSPICIOUS_DOMAINS = new Set([
    ...
    'evil-domain.xyz','phish-collect.ru','steal-creds.tk','malware-c2.ml',
    'botnet-ctrl.ga','ransom-pay.cf','credential-harvest.gq','exfil-server.pw',
    'c2-beacon.top','dropzone-upload.xyz','dark-payload.ru','cryptominer-js.tk',
    // TOR2WEB / C2 common patterns (partial)
    'onion.ly','onion.sh','tor2web.org','tor2web.fi','torstatus.blutmagie.de',
    // Known spam/phishing TLDs in abuse
    'free-prize.click','bank-verify.download','account-secure.zip',
    'paypal-security.cc','amazon-verify.biz','apple-support.ml',
    // GCC/Arab targeting phishing
    'stcpay-verify.xyz','absher-gov.tk','rajhi-bank.ga','alahli-secure.ml',
    'bank-alinma.cf','mada-verify.pw','sadad-payment.tk',
  ]);

  // Known malicious / Tor exit node IP prefixes (first 3 octets)
  const SUSPICIOUS_IP_PREFIXES = [
    '185.220.101','185.220.100','185.130.44','193.32.162','193.169.154',
    '45.142.212','45.141.84','89.234.157','91.108.4','5.188.10',
    '162.247.72','171.25.193','199.249.230','104.244.72','176.10.99',
    '51.15.43','51.38.81','109.70.100','185.100.86','192.42.116',
    '37.187.129','46.165.230','195.154.179','198.98.51',
  ];
  ...
  const PATTERNS = [
    ...
    { id:'ipv4',      cat:'network',  sev:'MEDIUM',   label:'IPv4 عنوان شبكي',
      ...
      enrich: v => { const pfx=v.split('.').slice(0,3).join('.'); return SUSPICIOUS_IP_PREFIXES.includes(pfx)?'⚠ IP مدرج في قوائم Tor/C2 المعروفة!':''; } },
    { id:'ipPrivate', cat:'network',  sev:'LOW',      label:'IP خاص (Private Network RFC1918)',
    ...
    { id:'domain',    cat:'network',  sev:'HIGH',     label:'نطاق ذو امتداد مشبوه',
      regex:/\b(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+(?:xyz|tk|ml|ga|cf|gq|pw|top|click|download|zip|ru|cn|cc|biz|work|life|online|site|fun|uno|men|racing|loan|win|bid|stream|trade|date|accountant|review|science)\b/gi,
      enrich: v => SUSPICIOUS_DOMAINS.has(v.toLowerCase())?'⚠ مدرج في قاعدة بيانات النطاقات الخبيثة!':'' },
    { id:'url',       cat:'network',  sev:'HIGH',     label:'رابط مُضمَّن (Embedded URL)',
      regex:/https?:\/\/[^\s\)<>"'\n]{10,}/g,
      enrich: v => { try { const h=new URL(v).hostname.toLowerCase(); return SUSPICIOUS_DOMAINS.has(h)?'⚠ النطاق مدرج في القائمة السوداء!':''; } catch { return ''; } } },
    { id:'torOnion',  cat:'network',  sev:'CRITICAL', label:'عنوان Tor .onion (Dark Web)',
      regex:/\b[a-z2-7]{16,56}\.onion\b/gi },
    { id:'pastebin',  cat:'network',  sev:'HIGH',     label:'رابط Pastebin / رفع نصي',
      regex:/https?:\/\/(?:pastebin\.com|paste\.ee|hastebin\.com|ghostbin\.com|dpaste\.com|rentry\.co)[^\s\n]*/gi },

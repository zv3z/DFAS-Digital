...
export const Route = createFileRoute("/about")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "نظام DFAS مبني وفق معايير ISO/IEC 27037 و NIST 800-86 و RFC 3227." },
      { property: "og:title", content: "DFAS · حول النظام" },
      { property: "og:description", content: "معلومات المشروع، المطوّر، والامتثال للمعايير الدولية." },
    ],
  }),
  component: About,
});
...
const STANDARDS = [
  { code: "ISO/IEC 27037:2012", title: "إرشادات التعامل مع الأدلة الرقمية", desc: "تحديد، تجميع، اقتناء، والمحافظة على الأدلة الرقمية." },
  { code: "NIST SP 800-86",     title: "دليل دمج الطب الشرعي الرقمي",        desc: "إطار عمل أمريكي شامل للتحقيق الجنائي الرقمي." },
  { code: "RFC 3227",            title: "Guidelines for Evidence Collection", desc: "المعيار العالمي لجمع الأدلة وأرشفتها بطريقة سليمة." },
];
...
function About() {
  return (
    <div className="px-6 lg:px-12 py-10 max-w-5xl mx-auto space-y-6">
      ...
      {/* TLP notice */}
      <div className="glass rounded-xl p-5 border-warning/40 flex items-start gap-4 animate-fade-up" style={{ boxShadow: "0 0 24px oklch(0.78 0.16 75 / 0.2)" }}>
        <div className="w-12 h-12 rounded-lg bg-warning/15 text-warning grid place-items-center text-2xl">⚠</div>
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2 py-0.5 rounded bg-warning/20 text-warning text-[10px] font-mono font-bold">TLP:AMBER</span>
            <span className="text-xs text-muted-foreground font-mono">Limited Disclosure</span>
          </div>
          <p className="text-sm">المخرجات والتقارير الناتجة عن النظام مصنّفة <strong className="text-warning">TLP:AMBER</strong>؛ يجوز مشاركتها فقط مع الأطراف المعنية بحدود الضرورة.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="glass rounded-xl p-6 animate-fade-up" style={{ animationDelay: ".05s" }}>
          ...
          <p className="text-xs text-muted-foreground mt-4 leading-relaxed">
            باحثون ومطوّرون متخصصون في الأمن السيبراني، التحقيق الرقمي، وتطوير أدوات التحليل المتقدمة.
          </p>
        </div>
      </div>
      ...
      <div className="glass rounded-xl p-6 animate-fade-up" style={{ animationDelay: ".1s" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
          {STANDARDS.map((s) => (
            <div key={s.code} className="rounded-lg border border-border bg-surface-2/50 p-4 hover:border-primary/40 transition">

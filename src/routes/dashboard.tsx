    ...
    <div>
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
              ...
              <div
                className="w-full rounded-md overflow-hidden bg-surface-2 flex flex-col justify-end"
                style={{ height: `${h}%` }}
              >
      ...
      <div className="flex items-center gap-4 mt-4 text-xs">
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-sm bg-critical" />
          حرج
        </span>
        <span className="flex items-center gap-1.5">
    ...
    <div className="px-6 lg:px-12 py-8 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          نظرة حيّة على نشاط التحليل وحالة التهديدات.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {METRICS.map((m, i) => (
          <div
            key={m.label}
            className="glass glass-hover rounded-xl p-5 animate-fade-up"
          ...
          >
            ...
            <div className={`mt-3 text-3xl font-bold text-${m.hue}`}>
              <Counter value={m.value} />
            </div>
            <div className="mt-1 text-xs text-muted-foreground font-mono">
              {m.delta} عن الأسبوع الماضي
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div
          className="glass rounded-xl p-6 lg:col-span-2 animate-fade-up"
      ...
      <div className="glass rounded-xl p-6 animate-fade-up" style={{ animationDelay: ".15s" }}>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/cases" className="text-xs text-cyan hover:underline">
            عرض الكل ←
          </Link>
        </div>
              ...
              {RECENT.map((r) => (
                ...
                >
                  ...
                  <td className="px-2 py-3 text-xs">
                    <span className="px-2 py-0.5 rounded bg-surface-2 font-mono">{r.mod}</span>
                  </td>
                  <td className="px-2 py-3">
                    <SeverityBadge level={r.sev} pulse />
                  </td>

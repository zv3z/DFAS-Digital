...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "DFAS v3 — 14 وحدة تحليل جنائي رقمي احترافية للمحققين السيبرانيين العرب.",
      },
    ...
    <div className="relative">
      ...
      >
        ...
        <div className="relative px-6 lg:px-12 py-16 lg:py-24 max-w-6xl mx-auto text-center">
          ...
          <h1
            className="text-5xl lg:text-7xl font-bold tracking-tight glow-text-cyan animate-fade-up"
            style={{ animationDelay: ".05s" }}
          >
            DFAS <span className="font-mono text-cyan">v3</span>
          </h1>
          <p
            className="mt-4 text-lg lg:text-xl text-foreground/80 animate-fade-up"
            style={{ animationDelay: ".15s" }}
          >
            نظام التحليل الجنائي الرقمي
          </p>
          <p
            className="mt-3 max-w-2xl mx-auto text-sm lg:text-base text-muted-foreground animate-fade-up"
            style={{ animationDelay: ".25s" }}
          >
            منصة احترافية للتحقيق الرقمي وتحليل الأدلة السيبرانية، مبنية على أحدث المعايير الدولية
            ومخصّصة للمحللين العرب.
          </p>
          ...
          >
            <Link
              to="/dashboard"
              className="px-6 py-3 rounded-lg bg-primary text-primary-foreground font-semibold glow-cyan hover:scale-[1.02] transition-transform"
            >
              فتح لوحة التحكم
            </Link>
      ...
      <section className="px-6 lg:px-12 py-10 max-w-7xl mx-auto">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {KPIS.map((k, i) => (
            <div
              key={k.label}
              className="glass glass-hover rounded-xl p-5 relative overflow-hidden animate-fade-up"
              style={{ animationDelay: `${i * 0.08}s` }}
            >
              <div
                className={`absolute -top-6 -left-6 w-24 h-24 rounded-full blur-2xl opacity-30 bg-${k.hue}`}
              />
              <div className="flex items-start justify-between relative">
                ...
                <div
                  className={`w-10 h-10 rounded-lg bg-${k.hue}/15 text-${k.hue} grid place-items-center text-lg`}
                >
                  {k.icon}
                </div>
              </div>
              <div className="mt-4 h-1 bg-surface-2 rounded-full overflow-hidden">
                <div
                  className={`h-full bg-${k.hue} animate-shimmer`}
                  style={{ width: `${60 + i * 10}%` }}
                />
              </div>
            </div>
      ...
      <section className="px-6 lg:px-12 pb-16 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-6">
          ...
          <Link to="/modules" className="text-sm text-cyan hover:underline">
            عرض الكل ←
          </Link>
        </div>
          ...
          {MODULES.map((m, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-surface-2 text-cyan border border-cyan/20">
                  {m.code}
                </span>
              </div>
